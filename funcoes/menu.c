...
void menuPrincipal(Figurinha *figurinhas, Figurinha *album, Figurinha *mochila, int total, int *total_album, int *total_mochila) {
    ...
    // estadoAtual vem da sua global.c, mas iniciamos no menu principal aqui por segurança
    ...
    while (!WindowShouldClose()) {
        ...
        // ==========================================
        // LÓGICA DE CLIQUES (GERENCIAMENTO DE TELAS)
        // ==========================================
        if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
            if (telaAtual == MENU_PRINCIPAL) {
                for (int i = 0; i < 7; i++) {
                    if (CheckCollisionPointRec(mousePoint, botoesPrincipal[i])) {
                        if (i == 0) telaAtual = ESTADO_ABRIR_PACOTE; // Ativa a tela gráfica nova!
                        else if (i == 1) telaAtual = MENU_INVENTARIO;
                        else if (i == 2) telaAtual = MENU_EXCLUIR;
                        else if (i == 3) acaoTerminal = 6;  // Pesquisar
                        else if (i == 4) acaoTerminal = 7;  // Alterar
                        else if (i == 5) acaoTerminal = 11; // Trocar
                        else if (i == 6) telaAtual = MENU_MINIGAMES;
                    }
                }
            } 
                ...
                for (int i = 0; i < 3; i++) {
                    if (CheckCollisionPointRec(mousePoint, botoesSubmenu[i])) {
                        if (i == 0) acaoTerminal = 2; // Ver Album
                        else if (i == 1) acaoTerminal = 3; // Ver Mochila
                        else if (i == 2) telaAtual = MENU_PRINCIPAL;
                    }
                }
            }
                ...
                for (int i = 0; i < 3; i++) {
                    if (CheckCollisionPointRec(mousePoint, botoesSubmenu[i])) {
                        if (i == 0) acaoTerminal = 4; // Excluir Album
                        else if (i == 1) acaoTerminal = 5; // Excluir Mochila
                        else if (i == 2) telaAtual = MENU_PRINCIPAL;
                    }
                }
            }
                ...
                for (int i = 0; i < 4; i++) {
                    if (CheckCollisionPointRec(mousePoint, botoesSubmenu[i])) {
                        if (i == 0) acaoTerminal = 8;  // Quiz
                        else if (i == 1) acaoTerminal = 9;  // Goleiro
                        else if (i == 2) acaoTerminal = 10; // Penalti
                        else if (i == 3) telaAtual = MENU_PRINCIPAL;
                    }
                }
            }
        }
        ...
        if (telaAtual == ESTADO_ABRIR_PACOTE) {
            abrirPacoteGrafico(figurinhas, mochila, album, total, total_mochila, total_album, fonteCopa, azulBrasil, amareloBrasil, &telaAtual);
        }
        // CASO CONTRÁRIO: Desenha as telas normais de menu
            ...
            const char *titulo; const char *subtitulo; const char **textosBotoes; int numBotoes = 3;

            if (telaAtual == MENU_INVENTARIO) {
                titulo = "MEU INVENTARIO"; subtitulo = "Qual colecao voce quer visualizar?"; textosBotoes = textosInventario;
            } else if (telaAtual == MENU_EXCLUIR) {
                titulo = "EXCLUIR FIGURINHAS"; subtitulo = "De onde voce quer remover figurinhas?"; textosBotoes = textosExcluir;
            } else { 
                titulo = "AREA DE MINIGAMES"; subtitulo = "Escolha seu desafio:"; textosBotoes = textosMinigames; numBotoes = 4;
            }
        ...
        // ==========================================
        // SISTEMA FALLBACK DE EXECUÇÃO EM TERMINAL
        // ==========================================
        if (acaoTerminal != 0) {
            ...
            else if (acaoTerminal == 4) excluirAlbum(figurinhas, album, total_album);
            else if (acaoTerminal == 5) excluirMochila(figurinhas, mochila, total_mochila);
            else if (acaoTerminal == 6) pesquisarFigurinha(figurinhas, total);
            ...
            else if (acaoTerminal == 11) trocarFigurinha(figurinhas, mochila, album, total_mochila, total_album);
            else if (acaoTerminal == 8) jogarQuiz(figurinhas, mochila, album, total, total_mochila, total_album);
            else if (acaoTerminal == 9) jogarGoleiro();
            else if (acaoTerminal == 10) jogarPenalti(figurinhas, mochila, album, total, total_mochila, total_album, &pacotes_fechados); 
            ...
            // Re-inicializa os componentes gráficos do motor
            InitWindow(1000, 800, "Album de Figurinhas - Copa do Mundo");
    ...
    // Fechamento e libertação de recursos ordenado
    UnloadTexture(cursorBola);
    UnloadFont(fonteCopa); 
    CloseWindow(); 
