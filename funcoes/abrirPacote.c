...
typedef struct{ // Estrutura para gerenciar a tela de abertura de pacotes
    ...
    bool modoTurbo;              // false = abre um pacote por vez; true = mostra todos de uma vez
                                 // funciona tipo um if else mas só pega um dos casos(ou é T ou é F)
...
static AberturaPacotes tela = {0}; // Inicializa a estrutura da tela de abertura de pacotes com valores padrão
                                    //Só funciona apenas nessa função 
...
void numeroParaTexto(int numero, char *destino){ //Converte um número inteiro (int) numa string de texto (char array)
    char invertido[20];
    int i = 0, j = 0;
...
void numeroParaTextoComDoisDigitos(int numero, char *destino){ //Formata um número garantindo que ele tenha sempre pelo menos dois dígitos.
...
static void descarregarRecursosAbertura(){ //Limpa a memória de vídeo libertando texturas e imagens que já não estão sendo usadas.
...
//Sorteia as cartas e classifica-as.
static void sortearFigurinhasDoPacote(Figurinha *figurinhas, Figurinha *mochila, Figurinha *album, int total, int *total_mochila, int *total_album, int qtdPacotes) {
    ...
    for (int f = 0; f < tela.totalCartasSorteioAtual; f++){
        ...
        if (!estaNoAlbum(album, *total_album, sorteada.codigo)){
            ...
            tela.statusCartas[f] = 1; // Nova Figurinha
        } else{
            ...
            salvarRepetida();
            tela.statusCartas[f] = 2; // Figurinha Repetida
        }
    }
}

//Prepara os dados (posição, velocidade, cor) de todas as partículas de confete.
...
//Atualiza cada partícula a cada frame (movimento + interação).
...
//Remove espaços inúteis no começo e no fim de uma string
...
//Tira todos os espaços dentro de um texto.
static void removerEspacosInternosImagem(char *texto){
    int j = 0;
    for(int i = 0; texto[i] != '\0'; i++){
        if (texto[i] != ' ' && texto[i] != '\t') { texto[j] = texto[i]; j++; }
    }
    texto[j] = '\0';
}

//Normaliza pastas que tenham espaços no nome (ex: "Africa do Sul").
...
//Converte uma string toda para MAIÚSCULAS.
...
//Converte uma string toda para minúsculas.
...
//Valida se uma string tem umcomeço. 
...
//Isola o número do código da figurinha.
static void pegarSufixoNumericoImagem(const char *codigo, char *sufixo){
    int i = 0;
    while (codigo[i] != '\0' && (codigo[i] < '0' || codigo[i] > '9')) { i++; }
    strcpy(sufixo, codigo + i);
}

//Cola duas strings para gerar um novo código possível na busca.
...
static void adicionarPastaImagem(char pastas[][100], int *totalPastas, const char *pasta){
    ...
    if (*totalPastas < 40) { strcpy(pastas[*totalPastas], pasta); (*totalPastas)++; }
}

static void adicionarCodigoImagem(char codigos[][30], int *totalCodigos, const char *codigo){
    if (codigo[0] == '\0') { return; }
    for (int i = 0; i < *totalCodigos; i++) { if (strcmp(codigos[i], codigo) == 0) { return; } }
    if (*totalCodigos < 40) { strcpy(codigos[*totalCodigos], codigo); (*totalCodigos)++; }
}

//Testa se o arquivo PNG existe no computador do jogador.
static int tentarCaminhoImagemFigurinha(char *caminho, const char *pasta, const char *codigo){
    ...
    strcat(caminho, ".png");
    if (FileExists(caminho)) { return 1; }
    return 0;
}

//gera várias combinações e vai tentando abrir as fotos até encontrar a certa (ex: Catar vs Qatar).
static int montarCaminhoImagemFigurinha(char *caminho, const char *secao, const char *codigo){
    ...
    int ehCongo = 0; int ehEstadosUnidos = 0; int ehQatar = 0; int ehSuica = 0; int ehFifa = 0;

    //Correção para mostrar as imagens no album que antes não aparecia
    if (strcmp(pastaUnderline, "Congo_DR") == 0) { ehCongo = 1; }
    if (strcmp(pastaUnderline, "Estados_Unidos") == 0 ) { ehEstadosUnidos = 1; }
    if (strcmp(pastaUnderline, "Qatar") == 0 ) { ehQatar = 1; }
    if (strcmp(pastaUnderline, "Suíça") == 0 ) { ehSuica = 1; }
    if (strcmp(pastaMaiuscula, "FIFA") == 0 ) { ehFifa = 1; }

    if (ehCongo) { adicionarPastaImagem(pastas, &totalPastas, "Congo_DR"); }
    if (ehEstadosUnidos) { adicionarPastaImagem(pastas, &totalPastas, "EUA"); }
    if (ehQatar) { adicionarPastaImagem(pastas, &totalPastas, "Catar"); }
    if (ehSuica) { adicionarPastaImagem(pastas, &totalPastas, "Suiça"); }
    if (ehFifa) { adicionarPastaImagem(pastas, &totalPastas, "FIFA_World_Cup_2026"); }
...
//Carrega a imagem local .png para dentro da Memória
...
//Estiliza a tela colocando gradientes escuros em cima e em baixo.
...
//Renderiza o estádio que fica no fundo da janela.
static void DesenharDecoracoesFutebol(float tempoGlobal){
    ...
    for(int x = 100; x < 900; x += 40){ DrawLineEx((Vector2){(float)x, 150}, (Vector2){(float)x + 20, 650}, 1.0f, Fade(WHITE, 0.04f)); }
    for(int y = 150; y < 650; y += 40){ DrawLineEx((Vector2){100, (float)y}, (Vector2){900, (float)y}, 1.0f, Fade(WHITE, 0.04f)); }
...
//Divide o gif do pacote ao meio para a animação de rasgar.
...
/*
 Divide o programa da tela abrir pacote:
 * Menu de Seleção (quantidade, modo turbo/normal).
 * Animação de Pré-Rasgo (Pacote a mexer/piscar).
 * Animação de Rasgar (Flash branco e divisão em dois do GIF).
 * Tela com Carrossel interativo exibindo todas as cartas.
 */
void abrirPacote(Figurinha *figurinhas, Figurinha *mochila, Figurinha *album, int total, int *total_mochila, int *total_album, Font fonteCopa, Color azulBrasil, Color amareloBrasil, EstadoMenu *estadoAtual) {
    ...
    // Lê o GIF pesado e atualiza as frames na memória para criar movimento.
    if (!tela.gifCarregado) {
    ...
    if (tela.fase == 0){
        ...
        // Limita a quantidade de entrada 
        ...
        //Renderização dos Retângulos de Botões (+, -, Max)
        ...
        //O "?" funciona como um atalho muito rápido e limpo para escrever um if e else numa única linha.
        /*ex:
        Color corDoBotao;

        if (seMenos){
         corDoBotao = Fade(COPA_VERMELHO, 0.2f);
         } else {
        corDoBotao = Fade(COPA_AZUL_ESCURO, 0.6f);
        }

        DrawRectangleRec(btnMenos, corDoBotao);*/
        ...
        //Lógica de manipulação dos cliques
        ...
        if (seAbrirNormal && IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) {
            if (pacotes_fechados > 0 && tela.quantidadeDesejada > 0) {
                ...
                pacotes_fechados--; // Deduz de 1 em 1
                salvarPacotes();
        ...
        if (seAbrirTurbo && IsMouseButtonReleased(MOUSE_LEFT_BUTTON)){
            if (pacotes_fechados > 0 && tela.quantidadeDesejada > 0){
                ...
                tela.modoTurbo = true;
                pacotes_fechados -= pacotesParaAbrir; // Deduz o valor Máximo escolhido
                salvarPacotes();
    ...
    //Animação pacote
        ...
        // Se ESPAÇO é pressionado, rasga o pacote de figurinhas
    ...
    //Animação Rasgar pacote
    } else if (tela.fase == 2){
        tela.tempoAnimacaoRasgar += GetFrameTime();
        float offsetRasgo = powf(tela.tempoAnimacaoRasgar * 3.5f, 2.0f) * 100.0f; // Física de gravidade na separação da textura
        Rectangle pacoteRec = {380, 130, 240, 450};
        ...
        if (tela.flashIntensidade > 0.0f){
            ...
            tela.flashIntensidade -= GetFrameTime() * 1.5f; // Decrementa o brilho
        }
    ...
    //Animação carrossel, estádio e placar com os dados
    } else if (tela.fase == 3){
        ...
        // As cartas inclinam-se de acordo com o lugar onde o mouse está!
        ...
        // Linha Colorida da Borda (Brilha Dourado se Rara)
        ...
        //Desenha os Textos de Identificação da Figurinha
        ...
        //Interface de Resumo (O que aconteceu com a carta - Álbum ou Mochila)
        if (tela.statusCartas[tela.cartaAtualIndice] == 1){
        ...
        //Setas(<-/->) para navegação caso haja múltiplas cartas
        ...
        //Apertar no Teclado para virar a carta 
